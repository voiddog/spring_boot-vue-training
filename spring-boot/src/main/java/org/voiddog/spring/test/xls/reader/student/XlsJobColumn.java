...
public class XlsJobColumn implements XlsColumn<StudentDAO> {

    private final String name = "职务";
    private final Pattern pattern = Pattern.compile("^\\s*((学员的?)?(职务|单位|职务))");
    ...
    public void readCell(Cell from, StudentDAO to) {
        to.setJob(XlsReadUtils.readString(from));
    }

    @Override
    public void writeCell(StudentDAO from, Cell cell) {
        cell.setCellValue(from.getJob());
    }
}
